...
export default function PropertyTabs() {
  ...
  return (
    <div className="w-full">
      {/* نوار تب‌ها */}
      <div className="flex flex-wrap gap-4 mb-2 mt-2 pb-2 border-b bg-secondary-light2 items-center rounded-md">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`rounded-full px-4 py-1 text-[12px] sm:text-sm md:text-sm lg:text-sm  font-medium transition-all duration-200 mt-2 ${
              activeTab === tab.id
