...
export default function PropertyTabs() {
  ...
  return (
    <div className="w-full">
      {/* نوار تب‌ها */}
      <div className="flex flex-wrap gap-3 mb-2 mt-2 border-b bg-secondary-light2 items-center pb-2 rounded-md">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 mt-2 ${
              activeTab === tab.id
      ...
      <div className="text-sm text-foreground leading-relaxed  p-4 rounded-lg">
        ...
        {activeTab === "features" && (
          <div className="flex flex-wrap justify-center gap-4 w-full">
            {features.map((item, index) => (
              <div
                key={index}
                className="w-36 h-24 bg-secondary-light2 flex flex-col gap-1 items-center justify-center p-3 rounded-xl text-[14px]"
              >
        ...
        {activeTab === "location" && (
          <p>اینجا نقشه یا موقعیت مکانی ملک قرار می‌گیرد...</p>
        )}
        {activeTab === "reviews" && (
          <p>نظرات کاربران درباره ملک در این بخش قرار می‌گیرد...</p>
        )}
      </div>
    </div>
  );
