      ...
      <Table className="text-right max-lg:hidden overflow-hidden">
        ...
        <TableBody>
          {payments.map((payment, idx) => (
            <TableRow key={idx}>
              <TableCell className="py-4 whitespace-nowrap">{payment.date}</TableCell>
              <TableCell className="whitespace-nowrap">{payment.phoneNumber}</TableCell>
              <TableCell className="whitespace-nowrap">
                {SplitNumber(payment.price)} {t('currency')}
              </TableCell>
              <TableCell>
                <div
                  className={`px-2 py-1 flex gap-2 whitespace-nowrap w-fit rounded-[16px] pl-6 items-center ${payment.status
                      ? 'bg-primary text-primary-foreground'
                      : 'bg-danger text-accent-foreground'
                    }`}
                >
                  {payment.status ? <CheckCircle2 size={14} /> : <X size={14} />}
                  {payment.status ? t('confirmed') : t('notConfirmed')}
                </div>
              </TableCell>
              <TableCell className="whitespace-nowrap">{payment.paymentType}</TableCell>
              <TableCell className="cursor-pointer whitespace-nowrap">{t('showReceipt')}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
            ...
            <div className=" h-full flex flex-col gap-2 max-sm:gap-4 text-base">
              ...
              <div className="flex gap-4 items-center flex-wrap">
                ...
                <span className="gap-2 flex">
                  {SplitNumber(payment.price)} <p>{t('currency')}</p>
                </span>
              </div>
              <div className="flex gap-4 items-center flex-wrap">
