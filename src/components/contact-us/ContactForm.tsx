        ...
        <form className={`flex flex-col gap-6 p-[16px]`} onSubmit={handleSubmit(onSubmit)}>
            <div className='flex gap-4 md:flex-row flex-col w-full'>
                <div className="flex flex-col gap-1 md:w-1/2 w-full group">
                    <Label htmlFor="name" className="text-[#AAAAAA] group-hover:text-card-foreground text-sm">
                        نام و نام خانوادگی:
                    </Label>
                    <Input
                        id="name"
                        {...register('name')}
                        className="border w-full outline-none border-[#AAAAAA] bg-transparent group-hover:border-white group-hover:placeholder:text-card-foreground px-4 py-2 placeholder:text-[#AAAAAA] rounded-2xl outline-0"
                        placeholder="وارد کنید..."
                    />
                    {errors.name && <p className='text-danger text-sm font-semibold'>{errors.name.message} </p>}
                </div>
                <div className="flex flex-col gap-1 md:w-1/2 w-full group">
                    <Label htmlFor="title" className="text-[#AAAAAA] group-hover:text-card-foreground text-sm">
                        عنوان:
                    </Label>
                    <Input
                        id="title"
            ...
            <div className="flex flex-col gap-1 group">
                <Label htmlFor="message" className="text-[#AAAAAA] group-hover:text-card-foreground text-sm">
                    پیام شما:
                </Label>
                <Textarea
                    id="message"
                    {...register('message')}
                    className="border h-[109px] outline-none border-[#AAAAAA] bg-transparent group-hover:border-white group-hover:placeholder:text-card-foreground px-4 py-2 placeholder:text-[#AAAAAA] rounded-2xl outline-0"
                    placeholder="....."
                />
