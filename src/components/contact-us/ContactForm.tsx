        ...
        <form className={`flex flex-col gap-6 p-[16px]`} onSubmit={handleSubmit(onSubmit)}>
            <div className='flex gap-4 md:flex-row flex-col w-full'>
                <div className="flex flex-col gap-1 md:w-1/2 w-full group">
                    <Label htmlFor="name" className="dark:text-subText text-primary-foreground group-hover:text-white text-sm">
                        نام و نام خانوادگی:
                    </Label>
                    <Input
                        id="name"
                        {...register('name')}
                        className="border w-full outline-none dark:border-subText border-primary-foreground bg-transparent group-hover:border-white group-hover:placeholder:text-white px-4 py-2 dark:placeholder:text-subText placeholder:text-primary-foreground rounded-2xl outline-0"
                        placeholder="وارد کنید..."
                    />
                    {errors.name && <p className='text-danger text-sm font-semibold'>{errors.name.message} </p>}
                </div>
                <div className="flex flex-col gap-1 md:w-1/2 w-full group">
                    <Label htmlFor="title" className="dark:text-subText text-primary-foreground group-hover:text-white text-sm">
                        عنوان:
                    </Label>
                    <Input
                        id="title"
            ...
            <div className="flex flex-col gap-1 group">
                <Label htmlFor="message" className="dark:text-subText text-primary-foreground group-hover:text-white text-sm">
                    پیام شما:
                </Label>
                <Textarea
                    id="message"
                    {...register('message')}
                    className="border h-[109px] outline-none dark:border-subText border-primary-foreground bg-transparent group-hover:border-white group-hover:placeholder:text-white px-4 py-2 dark:placeholder:text-subText placeholder:text-primary-foreground rounded-2xl outline-0"
                    placeholder="....."
                />
