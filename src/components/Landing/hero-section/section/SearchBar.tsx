...
import { Coins, MapPin, Search, X } from "lucide-react";
    ...
    <div className="absolute -bottom-2 lg:bottom-8 md:bottom-2 left-0 right-0 mx-auto w-full max-w-5xl z-50 px-4">
      ...
      <div className="md:hidden flex justify-center mt-10 items-center gap-2 ">
        <CommonButton
          ...
          classname={`bg-primary text-secondary p-2 text-xs rounded-xl ${
            isOpen ? "hidden" : "flex"
          }`}
          type="button"
          icon={<Search size={12} />}
      ...
      <div
        ref={searchRef}
        className={`flex justify-between items-center w-full bg-subBg backdrop-blur-sm rounded-[32px] p-4 transition-all duration-300 ${
      ...
      >
        ...
        <div
          className={`flex-1 flex flex-col md:flex-row items-center justify-around gap-4 w-full ${
