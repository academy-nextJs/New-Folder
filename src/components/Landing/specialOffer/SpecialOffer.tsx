...
const SpecialOffer = () => {
  ...
  const t = useTranslations('landing.specialOffer');
  ...
  return session?.accessToken && <div className="w-full h-fit bg-secondary-light px-8 py-12 rounded-[40px] relative">
    ...
    ">
      <div className="bg-danger text-card-foreground px-4 py-2 w-fit z-[1] absolute rounded-2xl flex gap-2 items-center right-[25px] top-[-35px]">
        <CountdownTimer initialSeconds={36000} classname='text-lg' /> <Clock size={16} className='text-white' />
      </div>

      <div className='flex flex-col gap-8'>
        <div className='flex flex-col gap-4'>
          <div className="flex flex-row w-fit items-center gap-4 justify-end">
            <span className="text-primary text-sm">{t('offersForYou')}</span>
          ...
          <div className='flex justify-between w-full items-center'>
            <h2 className='lg:text-[28px] md:text-xl text-base font-[500]'>{t('specialOffers')}</h2>
          </div>
        </div>
